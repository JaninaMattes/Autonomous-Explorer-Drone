    ...
    def collect_rollout(self, n_steps=1):
        ...
        while t_step < n_steps:
            ...
            for t_episode in range(0, self.max_batch_size):
                # render gym envs
                if self.render and t_episode % self.render_steps == 0:
                    self.env.render()
...
def make_env(env_id='Pendulum-v1', gym_wrappers=False, gui=DEFAULT_GUI, record_video=DEFAULT_RECORD_VIDEO, seed=42):
    ...
    env = gym.make(env_id)
    if env_id == "takeoff-aviary-v0":
        register_env("takeoff-aviary-v0", lambda _: TakeoffAviary())
        env = TakeoffAviary(gui=gui, record=record_video)                 
...
if __name__ == '__main__':
    ...
    env_name = 'takeoff-aviary-v0'       # name of OpenAI gym environment other: 'Pendulum-v1' , 'MountainCarContinuous-v0', 'takeoff-aviary-v0'
