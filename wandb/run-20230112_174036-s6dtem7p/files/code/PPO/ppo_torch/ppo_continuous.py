...
class PPO_PolicyGradient_V2:
    ...
    def learn(self):
        ...
        if self.stats_plotter:
            df = self.stats_plotter.read_csv(CURR_TIME) # just select files from current date
            self.stats_plotter.plot(df, x='timestep', y='mean episodic returns', hue='experiment', title=f'{env_name}')
...
def train(env, in_dim, out_dim, total_training_steps, max_batch_size, episode_iterations,
          ...
          log_video=False, ppo_version='v2', device='cpu', exp_path='./log/'):
...
if __name__ == '__main__':
    ...
    exp_folder_name = f'{LOG_PATH}_exp_{env_name}_{CURR_TIME}'
    ...
    # create CSV writer
    csv_writer = CSVWriter(f"{exp_folder_name}{env_name}_{CURR_TIME}.csv")
    stats_plotter = StatsPlotter(exp_folder_name, f'{env_name}_{CURR_TIME}.png', RESULTS_PATH)
