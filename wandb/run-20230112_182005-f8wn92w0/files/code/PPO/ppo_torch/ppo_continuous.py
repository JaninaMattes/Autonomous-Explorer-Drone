...
class PPO_PolicyGradient_V2:
    ...
    def log_stats(self, p_losses, v_losses, batch_return, batch_lens, steps, time, exp_name='experiment'):
        ...
        # calculate stats
        mean_p_loss = round(np.mean([np.sum(loss) for loss in p_losses]), 5)
        mean_v_loss = round(np.mean([np.sum(loss) for loss in v_losses]), 5)

        # Calculate the stats of an episode
        cum_ret = [np.sum(ep_rews) for ep_rews in batch_return]
        mean_ep_time = round(np.mean(time), 5) 
        mean_ep_len = round(np.mean(batch_lens), 5)

        # statistical values for return
        mean_ep_ret = round(np.mean(cum_ret), 5)
        max_ep_ret = round(np.max(cum_ret), 5)
        min_ep_ret = round(np.min(cum_ret), 5)

        # calculate standard deviation (spred of distribution)
        std_ep_rew = round(np.std(cum_ret), 5)
