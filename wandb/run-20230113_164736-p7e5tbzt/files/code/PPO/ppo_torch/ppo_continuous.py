...
class PPO_PolicyGradient_V2:
    ...
    def learn(self):
        ...
        while training_steps < self.total_training_steps:
            ...
            # break loop if better then defined threshold
            ep_reward = [np.sum(ep_rews) for ep_rews in rewards]
            running_reward = np.mean(ep_reward)
            if running_reward > self.env.spec.reward_threshold:
                print("Solved! Running reward is now {} and "
                        "the last episode runs to {} time steps!".format(running_reward, training_steps))
                break
        ...
        # Save any files starting with "ppo" as they're written to
    ...
    def log_stats(self, p_losses, v_losses, batch_return, batch_lens, training_steps, time, exp_name='experiment'):
        ...
        self.stats_data['timestep'].append(int(training_steps))
...
if __name__ == '__main__':
    ...
    # Hyperparameter
    total_training_steps = 3000     # time steps regarding batches collected and train agent
