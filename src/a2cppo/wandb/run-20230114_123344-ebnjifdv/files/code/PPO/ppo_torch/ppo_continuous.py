...
class PPO_PolicyGradient_V2:
    ...
    def advantage_TD_actor_critic(self, episode_rewards, obs, next_obs, normalized_adv=False, normalized_ret=False):
        """ Advantage TD Actor-Critic A(s,a) = r + (gamma * V(s_t+1)) - V(s_t)
            TD Error can be used as an estimator for Advantage function
        """
        next_values = self.get_value(next_obs).detach()
        values = self.get_value(obs).detach()

        advantages = []
        cum_returns = []
        for rewards in episode_rewards:  # reversed order
            for i in range(len(rewards)):
                # TD error: A(s,a) = r + (gamma * V(s_t+1)) - V(s_t)
                advantage = rewards[i] + (self.gamma * next_values[i]) - values[i]
                cum_returns.append(rewards[i]) # reverse it again
                advantages.append(advantage) # reverse it again
        cum_returns = torch.tensor(np.array(cum_returns), device=self.device, dtype=torch.float)
    ...
    def learn(self):
        ...
        while training_steps < self.total_training_steps:
            ...
            advantages, cum_returns = self.advantage_TD_actor_critic(rewards, obs, next_obs, normalized_adv=self.normalize_advantage, normalized_ret=self.normalize_return)
