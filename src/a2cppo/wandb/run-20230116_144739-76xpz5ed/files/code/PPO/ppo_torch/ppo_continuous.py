            ...
            for val in params:
                ...
                match key:
                    ...
                    case 'learning rate (value net)':
                        for i in range(0, len(val)):
                            wandb.config.update({"learning rate (value net)": val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'gamma (discount)':
                        for i in range(0, len(val)):
                            wandb.config.update({'gamma (discount)': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'epsilon (clip_range)':
                        for i in range(0, len(val)):
                            wandb.config.update({'epsilon (clip_range)': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'gae lambda (GAE)':
                        for i in range(0, len(val)):
                            wandb.config.update({'gae lambda (GAE)': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'epsilon (adam optimizer)':
                        for i in range(0, len(val)):
                            wandb.config.update({'epsilon (adam optimizer)': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'number of epochs for update':
                        for i in range(0, len(val)):
                            wandb.config.update({'number of epochs for update': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
                    ...
                    case 'max sampled trajectories':
                        for i in range(0, len(val)):
                            wandb.config.update({'max sampled trajectories': val})
                            train(env,
                                in_dim=obs_dim, 
                                out_dim=act_dim,
                                total_training_steps=total_training_steps,
