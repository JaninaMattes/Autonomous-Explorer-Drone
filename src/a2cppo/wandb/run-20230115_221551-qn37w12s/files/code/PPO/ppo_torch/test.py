...
def simulate_rollout(policy_net, env, render=True):
    # Rollout until process ends
	while True:
		obs, done, frames = env.reset(), False, []
		# Logging
...
def test(model_path, env, in_dim, out_dim, n_steps=1, device='cpu', log_video=True, filename='pendulum_v1.gif'):
    ...
    frames = []
    for ep_num in range(0, n_steps):
        ep_lengths, ep_returns, frames = simulate_rollout(policy_net, env, render)
    ...
    if log_video:
        wandb.log({"test/video": wandb.Video(os.path.join(base_path, filename), caption='episode: '+str(ep_num), fps=4, format="gif"), "step": ep_num})
...
if __name__ == '__main__':
    ...
    test(args.model_path, env, in_dim=obs_dim, out_dim=act_dim, n_steps=1000, device=device)