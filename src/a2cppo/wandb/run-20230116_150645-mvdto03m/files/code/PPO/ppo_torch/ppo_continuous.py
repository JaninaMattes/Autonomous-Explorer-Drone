...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        param_dict = {
                ...
                'gae lambda (GAE)': [0.9, 0.93, 0.96, 0.99, 1.0],
                'epsilon (adam optimizer)': [1e-9, 1e-8, 1e-7, 1e-6, 1e-5],
                'number of epochs for update': [8, 16, 32, 64],
                'max sampled trajectories': [32, 64, 128, 256, 512]
        }
            ...
            match key:
                    case 'learning rate (policy net)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({
                                "learning rate (policy net)": val[i],
                    ...
                    case 'learning rate (value net)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({"learning rate (value net)": val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                            train(env,
                    ...
                    case 'gamma (discount)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'gamma (discount)': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                            train(env,
                    ...
                    case 'epsilon (clip_range)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'epsilon (clip_range)': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                    ...
                    case 'gae lambda (GAE)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'gae lambda (GAE)': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                    ...
                    case 'epsilon (adam optimizer)':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'epsilon (adam optimizer)': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                    ...
                    case 'number of epochs for update':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'number of epochs for update': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
                    ...
                    case 'max sampled trajectories':
                        for i in range(0, len(val)):
                            run_name = f"{args.exp_name} -hyperparam {key}: {val[i]}"
                            wandb.config.update({'max sampled trajectories': val[i],
                                                "experiment name": run_name},
                                                allow_val_change=True)
                            wandb.init(name=run_name)
